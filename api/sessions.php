...
turso([['sql'=>"CREATE TABLE IF NOT EXISTS sessions(id TEXT PRIMARY KEY,data TEXT NOT NULL,created_at TEXT DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')))"]]);
...
try {
    if($method==='GET'&&!$id&&!isset($_GET['maxid'])){
        ...
        turso([['sql'=>"INSERT INTO sessions(id,data)VALUES(:id,:data)",'args'=>[':id'=>$body['id'],':data'=>json_encode($body)]]]);
        http_response_code(201);echo json_encode(['ok'=>true,'id'=>$body['id']]);

    }elseif($method==='PATCH'&&$id){
        $body=json_decode(file_get_contents('php://input'),true);
        $res=turso([['sql'=>"SELECT data FROM sessions WHERE id=:id",'args'=>[':id'=>$id]]]);
        $rows=$res[0]['response']['result']['rows']??[];
        ...
        turso([['sql'=>"UPDATE sessions SET data=:data WHERE id=:id",'args'=>[':data'=>json_encode($ses),':id'=>$id]]]);
        echo json_encode(['ok'=>true]);

    }elseif($method==='DELETE'&&$id){
        turso([['sql'=>"DELETE FROM sessions WHERE id=:id",'args'=>[':id'=>$id]]]);
        echo json_encode(['ok'=>true]);
